...
expect(validatePassword("aaabbbcc")).toBe("Password should contain minimum 1 character for both uppercase and lowercase letter");
expect(validatePassword("AAABBBCC")).toBe("Password should contain minimum 1 character for both uppercase and lowercase letter");
});


test("Password should contain minimum 1 digit of numeric value", () => {
    expect(validatePassword("Aaabbbcc")).toBe("Password should contain minimum 1 digit of numeric value");
});


test("Password should contain minimum 1 special character", () => {
    expect(validatePassword("Aaabbbcc1")).toBe("Password should contain minimum 1 special character");
});
